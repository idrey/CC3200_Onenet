...
#define SERVER_ADDRESS          "mqtts.heclouds.com"
...
//需要修改
#define PUB_TOPIC_FOR_SW2       "$sys/439074/cc3200/dp/post/json"
...
//需要修改
#define TOPIC1                  "$sys/439074/cc3200/cmd/request/#"

#define RECV_MSG				"1234"
/*Defining QOS levels*/
#define QOS0                    0
#define QOS1                    1
#define QOS2                    2
...
//char *security_file_list[] = {"/cert/ca.der"};
char *devStr = "0x41";
...
{
    {
        ...
        "cc3200", //需要修改
        "439074", //需要修改
        "version=2018-10-31&res=products%2F439074&et=1624541723&method=sha1&sign=o7AzFrh0vSbrZz9vGbm9cLmd%2FX8%3D", //需要修改
        true,
        KEEP_ALIVE_TIMER,
...
unsigned int
ProcessReadRegCommand(char *regStr)
{
...
//    UART_PRINT("I2C Read From address complete\n\r");
    ...
    //
    unsigned int f = (unsigned int)aucRdDataBuf[0];
    unsigned int s = (unsigned int)aucRdDataBuf[1];
    unsigned int ans = f;
    ans = ans << 8;
    ans = ans + s;
//    UART_PRINT("%d   %x\n\r", ans, ans);
...
//         Subscribe to topics
    ...
    unsigned int temp_die = 0;
    float temp = 0;
    do {
        config = ProcessReadRegCommand(regStr);
    }while((config & 0x0080) != 0x0080);
    UART_PRINT("GOTO PUBLISH\n\r");
    for(;;)
    {
        osi_MsgQRead( &g_PBQueue, &RecvQue, OSI_NO_WAIT);
...
//            //
//            data_sw2 = (unsigned char*)getJson(15);
...
//        }
        ...
        if(flag_red == 1) {
            ...
            temp_die = ProcessReadRegCommand(regTemp);
            temp = (float)(temp_die >> 2)*0.03125;
            sprintf(temp_str, "%.2f", temp);
            UART_PRINT("Temp is %s\r\n", temp_str);
...
char *getJson(char* temp) {
    ...
    /* ´´½¨Ò»¸öJSONÊý¾Ý¶ÔÏó(Á´±íÍ·½áµã) */
    cjson_test = cJSON_CreateObject();

    /* Ìí¼ÓÒ»Ìõ×Ö·û´®ÀàÐÍµÄJSONÊý¾Ý(Ìí¼ÓÒ»¸öÁ´±í½Úµã) */
